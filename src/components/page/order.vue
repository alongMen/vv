<template>
    <div>
        ...
        <el-card class="card-item" v-for="(item,index) in list" :key="index" > 
            ...
            <div class="right">
                <p>{{item.title}}</p>
                <p style="color:rgba(0,0,0,0.4)">订单编号：{{item.bid}}</p>
                <p style="color:rgba(0,0,0,0.4)">¥ 100 (单价) * {{item.buystock}} (数量) = ¥ {{item.price}} (总计) </p>
                <p style="color:rgba(0,0,0,0.4)">收货地址：北京市--联系方式 ({{item.phone}})</p>
                <p class="status">订单状态：{{item.buystatus}}</p>
            </div>
        </el-card>
    </div>
</template>
